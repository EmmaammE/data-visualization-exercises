(function () {
        ...
        $ele
            ...
            .on('mouseover', function (d, i) {
                ...
                let chapter_ = Math.round(
                    cor[0] / ((template.width - 2 * template.padding) / 120)
                );
    ...
    function renderAllLegends(chapterLegend,data) {
        if (chapterLegend !== undefined) {
            ...
            legends.append('p').attr("class","number").text(d => d[1]);
    ...
    // define scale & axis
        ...
        yScale_ = d3.scaleLinear().domain([d3.min(sum,d=>d.chapter),d3.max(sum,d=>d.value)])
            .range([timeline.height - template.padding,template.padding]);
        yAxis_ = d3.axisLeft().scale(yScale_).ticks(8);
        ...
        let area = d3.area()
            ...
            .y0(yScale_(0)).curve(d3.curveCatmullRom.alpha(0.5));
